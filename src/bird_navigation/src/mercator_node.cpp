...
public:
  Mercator()
    ...
    map_pub_ =
        this->create_publisher<nav_msgs::msg::OccupancyGrid>(output_topic_, 10);
    ...
    timer_ = this->create_wall_timer(
        ...
        std::bind(&Mercator::publish_map_to_odom, this));
    RCLCPP_INFO(this->get_logger(),
                "Mercator initialized with initial transform published.");
  }
...
private:
  ...
  void integrate_grid(const nav_msgs::msg::OccupancyGrid &local_grid,
    ...
    double local_resolution =
        local_grid.info.resolution; // local grid resolution
    double master_resolution =
        master_map_->info.resolution; // master map resolution
    int global_width = master_map_->info.width;
    int global_height = master_map_->info.height;

    // Extract yaw from the transform's quaternion.
    double yaw = tf2::getYaw(transform.transform.rotation);
    double cos_yaw = std::cos(yaw);
    double sin_yaw = std::sin(yaw);

    // Iterate through each cell in the local grid.
    for (int y = 0; y < local_height; ++y) {
      for (int x = 0; x < local_width; ++x) {
        // Compute the local cell center (accounting for the local grid's
        // origin)
        double local_cell_x =
            local_grid.info.origin.position.x + (x + 0.5) * local_resolution;
        double local_cell_y =
            local_grid.info.origin.position.y + (y + 0.5) * local_resolution;

        // Apply rotation and translation from the transform to get the global
        // (map) coordinates.
        double gx = transform.transform.translation.x +
                    (cos_yaw * local_cell_x - sin_yaw * local_cell_y);
        double gy = transform.transform.translation.y +
                    (sin_yaw * local_cell_x + cos_yaw * local_cell_y);

        // Convert the global coordinate to master map indices.
        int mx = static_cast<int>((gx - master_map_->info.origin.position.x) /
                                  master_resolution);
        int my = static_cast<int>((gy - master_map_->info.origin.position.y) /
                                  master_resolution);

        // Check bounds before updating the master map.
        if (mx >= 0 && mx < global_width && my >= 0 && my < global_height) {
          int idx_local = y * local_width + x;
          int idx_global = my * global_width + mx;
          int8_t val =
              local_grid.data[idx_local]; // -1: unknown, 0-100: occupancy
          ...
          if (val != -1) {
            ...
            master_map_->data[idx_global] =
                logOddsToOccupancy(master_log_odds_[idx_global]);
          }
        }
      }
    }
